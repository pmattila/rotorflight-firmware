...
static bool osdDisplayStat(int statistic, uint8_t displayRow)
{
    ...
    case OSD_STAT_TOTAL_DIST:
        ...
        if (osdConfig()->units == UNIT_IMPERIAL) {
            tfp_sprintf(buff, "%d%c", (int)statsConfig()->stats_total_dist_m / METERS_PER_MILE, SYM_MILES);
        } else {
            tfp_sprintf(buff, "%d%c", (int)statsConfig()->stats_total_dist_m / METERS_PER_KILOMETER, SYM_KM);
        }
