...
static void showGpsPage(void)
{
    ...
    tfp_sprintf(lineBuffer, "RX: %ld", GPS_packetCount);
    padHalfLineBuffer();
    i2c_OLED_set_line(dev, rowIndex);
    i2c_OLED_send_string(dev, lineBuffer);
    ...
    tfp_sprintf(lineBuffer, "Dt: %ld", gpsData.lastMessage - gpsData.lastLastMessage);
    padHalfLineBuffer();
    i2c_OLED_set_line(dev, rowIndex);
    i2c_OLED_send_string(dev, lineBuffer);
...
static void showBatteryPage(void)
{
    ...
    if (batteryConfig()->currentMeterSource != CURRENT_METER_NONE) {
        ...
        // Amp: DDD.D mAh: DDDDD
        tfp_sprintf(lineBuffer, "Amp: %ld.%ld mAh: %ld", amperage / 100, (amperage % 100) / 10, getBatteryCapacityUsed());
        padLineBuffer();
        i2c_OLED_set_line(dev, rowIndex++);
        i2c_OLED_send_string(dev, lineBuffer);
